...
**УВАГА! На даний момент проект не слідкує за змінами внесеними у поточний проект анотацій. Отож проект анотацій буде закрито без будь-яких попереджень з наступною втратою даних! Необхідно вручну зберегти зміни внесені в проект і тільки після цього його закрити.**

# Вимоги

Дана секція містить список усіх вимог до пакунків, програм чи інструментів які повинні бути встановленими у системі для того щоб побудувати проект чи виконати інші функції.
...
Якщо необхідно побудувати і виконати доступні тести необхідно встановити GTest або забезпечити доступ до мережі Інтернет у випадку самостійного встановлення скриптами системи збірки CMake поточного проекту. Для встановлення необхідно виконати команду:
...
Необіхдно замінити `org.gnome.Platform/x86_64/48 org.gnome.Sdk/x86_64/48` власним варіантом обраного середовища і SDK. У документації до flatpak можна визначити як переглянути усі доступні варіанти.
...
Для того щоб мати змогу будувати і запускати контейнери Docker разом з побудовою поточного проекту його необхідно встановити за допомогою команди (ОС на базі GNU/Linux):
...
## Побудова проекту через командний рядок

Проект використовує систему побудови CMake - отож команди побудови мають бути знайомими більшосі C++ розробників на планеті!

Звісно, проект спочатку потребує клонування за допомогою команди `git clone <URL>` і його коренева директорія повинна бути відкритою у використовуваному IDE. Після чого можна виконати команди:
...
Яка у свою чергу створить директорію `build` (директорія вже додана до файлу `.gitignore`), конфігурує проект використовуючи доступну у ОС версію системи побудови CMake (детальніше у секції [Вимоги](#вимоги) даного документу), і на кінець, будує усі цілі доступні у проекті.

## Вмикання тестів

### Вмикання юніт-тестів

Для того щоб зробити юніт тестування доступним (для побудови і запуску) необхідно перекорфігурувати проект з увімкненою опцією `ENABLE_UNIT_TESTS` за допомогою команди (для GNU/Linux):
...
### Вмикання компонент-тестів

Для того щоб зробити компоненті тести доступними для побудови і запуску, необхідно перекорфігурувати проект з увімкненою опцією `ENABLE_COMPONENT_TESTS` за допомогою команди (для GNU/Linux):
...
Яка в свою чергу створить директорію з назвою `build` всередині кореневої директорію проекту, відкриє її за допомогою команди `cd` і сконфігурує проект з увімкненою ціллю для побудови Doxygen документації.
...
Яка у свою чергу створює директорію `doc/ImagesAnnotator-html` (вже додана до файлу `.gitignore`) що буде містити зібрану документацію у вигляді HTML-сторінок і ресурсів до них. Для того щоб відкрити і оглянути згенеровану документацію необхідно відкрити файл який лежить за шляхом `doc/ImagesAnnotator-html/index.html` від кореня проекту. Директорія `ImagesAnnotator-html` змінить назву у відповідності до зміни назви головного виконуваного файлу проекту за допомогою встановлення нового значення для змінної `PROJECT_BINARY_NAME` у файлі `cmake/template-project-misc-variables-declare.cmake` або змінну `DOXYGEN_OUT_HTML_NAME` яка визначає ім'я для цілої цільової директорії для згенерованої HTML-документації.
...
Наявна можливість підтримки встановлення документації за допомогою встановлення змінних `ENABLE_DOC_DOXYGEN` і `DOXYGEN_DO_INSTALL` у значення `ON` під час конфігурування прокету за допомогою команди:
...
Для того щоб зробити доступною ціль `clang-format` необхідно встановити змінну `ENABLE_CLANGFORMAT` у значення `ON` під час конфігурації проекту (для GNU/Linux ОС):

```
# всередині кореня проекту

mkdir -vp build && cd build && cmake ../ -DENABLE_CLANGFORMAT=ON
```

Для того щоб виконати форматування коду усього проекту у відповідності до стандартів вказаних у файлі `misc/.clang-format` необхідно виконати наступну команду:
...
**Попередження! Ціль запустить на виконання головний виконуваний файл проекту.** Динамічна перевірка пам'яті виконуваного файлу проекту вимагає запуск програми і повний прохід виконання. Необхідно забезпечити кінечність виконання програми щоб запобігти невизначено довгий час перевірки.
...
На кінець, необіхдно виконати команду `cpack` всередині директорії побудови проекту:
...
Згенерований пакет повинен згенеруватись всередині кореня директорії побудови проекту. Наприклад, якщо ім'я проекту було не зміненим і його версія 0.8.0 тоді назва пакету може виглядати наступним чином: `ImagesAnnotator-0.8.0-Linux.deb`.
...
У разі успішної побудови цілі буде створено файл під назвою `ImagesAnnotator-0.9.0.flatpak` у кореневій директорії побудови проекту. Для того щоб преглянути або/і змінити ціль побудови пакету flatpak необхідно відвідати файли `cmake/template-project-flatpak-target.cmake` або `misc/flatpak.conf.json.in`.
...
Щоб увімкнути і запустити Docker контейнер разом з цільовим бінарним виконуваним файлом поточного проекту необхідно встановити у значення `ON` змінну `ENABLE_DOCKER` за допомогою команди:
...
Ім'я `ImagesAnnotator` являється ім'ям головного виконуваного бінарного файлу за замовчуванням. Його необхідно замінити у разі заміни ім'я за умовчанням у кореневому файлі `CMakeLists.txt` (змінні `PROJECT_NAME` або `PROJECT_BINARY_NAME`).
...
І ще раз, ім'я `ImagesAnnotator` являється ім'ям головного виконуваного бінарного файлу за замовчуванням. Його необхідно замінити у разі заміни ім'я за умовчанням у кореневому файлі `CMakeLists.txt` (змінні `PROJECT_NAME` або `PROJECT_BINARY_NAME`).
...
Як альтернатива, можна виконати команду `ctest` з будь-якого розміщення у файловій системі за допомогою абсолютного шляху до директорії побудови проекту (для GNU/Linux ОС і подібних):
